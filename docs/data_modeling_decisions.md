...
## Docker Build-Time Secret Handling — dbt Image

**Constraint:** the dbt image's build step (`dbt parse`, used to generate the manifest consumed by Cosmos's `LoadMode.MANIFEST`) requires a live Databricks token to resolve `profiles.yml` and connect. A standard `ARG`/`ENV` value passed at `docker build` time persists into the image's layer history — visible in plaintext via `docker history` or `docker inspect` to anyone with access to the built image. This is inconsistent with this project's standing no-plaintext-secrets discipline, applied consistently elsewhere (gitignored Terraform state, OAuth-based `databricks-connect` and CLI-profile auth, `config_variables.yml`-sourced credentials for Great Expectations).

**Decision: BuildKit secret mount (`--mount=type=secret`), not `ARG`/`ENV`.** The token is made available only inside the specific `RUN` step that needs it, as a tmpfs file at `/run/secrets/dbt_token` — never written to any image layer, never recoverable from the built image after the fact. `profiles.yml` itself is staged into the Docker build context by `scripts/build_manifest.ps1` from its canonical location (`~/.dbt/profiles.yml`, outside the repository) and removed immediately after the build completes; it is safe to stage transiently because it templates the token via `env_var()` rather than containing it directly.

**Scope of this fix:** it applies only to the build-time `dbt parse` step. At container runtime — when Cosmos's Docker execution mode launches this same image to run an actual model — the token is passed as a normal runtime environment variable, the same pattern already used for `databricks-connect` elsewhere in the stack. Runtime environment variables are not written to any image layer regardless of mechanism, so no BuildKit-specific handling is required at that stage; the risk this decision addresses is specific to values baked in during `docker build`, not values injected at `docker run`.

---

## Cosmos for dbt Orchestration — Execution Mode and Manifest Loading

**Decision: `astronomer-cosmos` over hand-written `BashOperator`/`DockerOperator` tasks per dbt command.** Cosmos parses the dbt project and auto-generates one Airflow task per dbt model, mirroring dbt's own dependency graph, rather than wrapping the whole project in a single opaque `dbt run` task. This gives per-model retries, per-model status in the Airflow UI, and a lineage graph (`dbt_lineage_dag.py`'s Graph view) that directly reflects the underlying dbt project structure — consistent with this project's general preference for a tool whose native abstraction matches the problem, over a thinner wrapper that would hide dbt's own DAG behind a single task boundary.

**Decision: `ExecutionMode.DOCKER`, not local or virtualenv execution.** Running dbt inside its own container (`financial-pipeline-dbt:latest`, built by `Dockerfile.dbt`) isolates dbt's dependency chain (`dbt-databricks`, `dbt-core`, `dbt_packages`) from Airflow's own environment. This turned out to be the right call for a very concrete reason discovered later in the same phase: `financial_pipeline_dag.py`'s own image (`Dockerfile.airflow`) hit a severe pip dependency-resolution conflict trying to install `databricks-connect` alongside Airflow's pinned dependency stack (see "Airflow Image Dependency Isolation" below) — and `dbt-databricks` pulls an even larger, independently-versioned Databricks toolchain (`dbt-core`, `dbt-spark`, `dbt-adapters`, `databricks-sql-connector`) that would collide with both of those. Keeping dbt execution in its own container sidesteps that entire class of conflict rather than fighting it inside a single shared Airflow image.

**Decision: `LoadMode.MANIFEST`, not `LoadMode.DBT_LS` at DAG-parse time.** The dbt project is parsed once, at image-build time (`dbt parse` inside `Dockerfile.dbt`, via `scripts/build_manifest.ps1`), producing `manifest.json`, which Cosmos then reads to build the Airflow task graph. The alternative — `LoadMode.DBT_LS`, which shells out to `dbt ls` at DAG-parse time — would re-parse the dbt project on every scheduler DAG-refresh cycle and would require the scheduler to have live Databricks connectivity just to determine task structure, not just to execute tasks. Manifest-based loading keeps DAG parsing fast, offline, and decoupled from warehouse availability; the trade-off is that the manifest must be regenerated (`scripts/build_manifest.ps1`) and the image rebuilt whenever a dbt model changes, rather than picking up changes automatically.

---

## Runtime Credentials for Docker-Execution-Mode Cosmos Tasks

**Decision: `ProfileConfig` uses `profiles_yml_filepath`, not `ProfileMapping`.** Under `ExecutionMode.DOCKER`, Cosmos raises `CosmosValueError` if both are combined — the isolated dbt container has no access to Airflow's Connection store (`databricks_default`), since it's a separate container, not a Python process inside the scheduler. Credentials instead have to be baked into the dbt image itself, via the same `profiles.yml` already staged into the build context for the BuildKit-secret build step documented above. `databricks_default` remains a real, deliberately-created Airflow Connection (encrypted, documented) — it's just not what this DAG's execution mode actually uses; it stands as a documented option for a future non-Docker Cosmos execution mode, where Airflow's own Connection store would be reachable.

**Decision: build-time and runtime credentials are two separate delivery mechanisms for the same image, not one.** The BuildKit secret mount only protects `dbt parse` at build time. At runtime, when Cosmos's `DockerOperator`-generated tasks launch this image to actually run a model, `DBT_DATABRICKS_TOKEN` is passed as a normal container environment variable (`operator_args["environment"]` in `dbt_lineage_dag.py`), sourced from the Airflow scheduler's own process environment — which itself needs `DBT_DATABRICKS_TOKEN: ${DBT_DATABRICKS_TOKEN}` in `docker-compose.yml`'s `x-airflow-common` block, populated from the host shell at `docker compose up` time. Two different credential-delivery mechanisms for two different lifecycle stages of the same image — build-time secrets need BuildKit's protection because they'd otherwise land in a committed layer; runtime environment variables never touch an image layer regardless of mechanism, so no special handling is needed there.

---

## Airflow Image Dependency Isolation — Layered pip Installs, Not `requirements.txt`

**Constraint:** `financial_pipeline_dag.py`'s tasks call `src/extract.py`, `src/load.py`, `src/transform.py`, `src/transform_dim_securities.py`, and `src/apply_corrections.py` directly via `PythonOperator`, so `Dockerfile.airflow` needs `databricks-connect`, `pandas`, `yfinance`, `requests`, `python-dotenv`, and `jsonschema` installed alongside Airflow itself.

**Rejected: `pip install -r requirements.txt` in a single invocation.** This hung for 28+ minutes in exponential dependency backtracking and was cancelled without resolving. `requirements.txt` also includes `dbt-databricks` — deliberately isolated into the separate `Dockerfile.dbt` image already (see above) — along with `great_expectations` and `databricks-sqlalchemy`, neither used by any DAG task. Installing all of it in one pip call put two independently-versioned Databricks toolchains (`dbt-databricks`'s chain and `databricks-connect`) in the same resolution, each pinning different `grpcio`/`grpcio-status`/`googleapis-common-protos` ranges, with no early exit for pip's resolver.

**Trimming the package list alone didn't fully fix it.** Even reduced to just the packages actually imported by `src/`, installing `databricks-connect` and `yfinance` in the same pip call kept backtracking through `grpcio-status` versions for an extended period — pip's resolver handles this style of diamond dependency (two packages each pinning a different sub-dependency range) poorly regardless of how few packages are involved.

**Decision: separate `RUN` layers, one package/group per layer**, rather than one combined install — Airflow and its providers in one layer, `databricks-connect` alone in a second, the plain-Python packages (`pandas`, `yfinance`, `requests`, `python-dotenv`, `jsonschema`) together in a third. Each layer resolves the new package(s) only against what's already installed on disk (fixed, no backtracking) instead of jointly resolving multiple new packages against each other. This is a general Docker/pip pattern for this class of conflict, not specific to Databricks tooling. Build time dropped from indefinite (cancelled after 28+ minutes) to under 30 seconds.

---

## Headless Container Auth — PAT Profile, Not CLI-Based OAuth

**Constraint:** `get_spark_session()` (`src/load.py`) opens a Databricks Connect session via `DatabricksSession.builder.serverless().profile(profile)` — a profile-based lookup against `~/.databrickscfg`. The project's existing local-dev profile (`financial_market_data_pipeline`) uses `auth_type = databricks-cli`, meaning the SDK shells out to the `databricks` CLI binary to refresh an OAuth token. That works on a developer machine with the CLI installed and already logged in, but fails inside the Airflow container: no CLI binary is installed there, so credential resolution raises `PermissionError` attempting to even stat a nonexistent CLI path, surfaced by `load.py`'s own wrapped `EnvironmentError`.

**Decision: a second, PAT-based profile (`airflow_pat`) in `~/.databrickscfg`**, alongside — not replacing — the existing CLI-based `financial_market_data_pipeline` profile still used for local CLI/dbt work. `airflow_pat` sets `host` + `token` (a workspace PAT minted via `databricks tokens create`, itself authenticated using the already-working CLI-based local profile) + `serverless_compute_id = auto`, with no `auth_type` line — a bare `token` field is sufficient for PAT resolution, with no CLI dependency at all. Selected via `DATABRICKS_CONFIG_PROFILE: airflow_pat` in `docker-compose.yml`'s `x-airflow-common` environment block; `get_spark_session()` already reads this env var as an override before falling back to its `DEFAULT_PROFILE` constant, so no code changes were needed — the extensibility point had already been built in, ahead of actually needing it.

Same category of decision as the dbt image's build-time-vs-runtime credential split above, just one layer down: local development and headless container execution need different auth mechanisms for the same underlying service, and the fix is a second, purpose-built profile rather than forcing one mechanism to serve both contexts.
